...
RegisterDateTimeActions() {
    ...
    RegisterAction("Convert Date Format", "📅 Date/Time", "Replaces Indian date with remembered default (or prompts if unselected)", "date, convert, convertor, format, transform, indian, standard", (*) => ConvertSelectedDateInPlace())
    RegisterAction("Configure Default Date Format", "⚙️ Settings", "Set everyday default format (saved to office_productivity_settings.ini)", "default date, defaults date, date default, date defaults, preset date, initial date, starting date, base date, standard date, selected date, suggested date, automatic date, predefined date, initialised date, reference date, assigned date, preselected date, date standard", (*) => (IsSet(ShowDateFormatSettingsGui) ? ShowDateFormatSettingsGui() : ""))
