...
try {
    ...
    ; NOTE: DEFECT-038 through DEFECT-041 (CorpusSetEngine, delimiter preservation, RepeatLastAction
    ; toast preservation, and unified visual feedback epoch) are intentionally NOT present on master.
    ; They depend on Lib/CorpusSetEngine.ahk and the "#Include CorpusSetEngine.ahk" wiring in
    ; Lib/Actions_Text.ahk / Lib/ToolAdapters_Builtin.ahk, which are not part of master yet (they ship
    ; in the still-unmerged "feat/corpus-set-engine" branch / PR #4). An earlier commit on this branch
    ; accidentally included this block without its dependency, which made master's test suite crash
    ; with "This global variable has not been assigned a value" on CorpusSetEngine.Analyze() — this
    ; hotfix removes the orphaned block to restore a passing test suite. The block itself is not lost:
    ; it will be reintroduced by PR #4 once CorpusSetEngine.ahk is available in this test file's
    ; #Include chain.
